...
## 후속 — 패턴 D-2 적용 (커밋 `0876080f`)

DOW-1161에서 알림·확인·입력·이동 4개 패턴이 확정됐다. 그 문서 3절 적용 표의 16번이 이 화면이고, C-1(인라인 패널)·A(toast)는 위 작업에서 이미 맞았으며 **D-2만 남아 있었다.**

`data?.hidden`일 때 `toast.success` 직후 `router.push('/')`를 부르던 부분을 없앴다. 3초짜리 토스트와 화면 전환이 겹쳐 사용자는 둘 중 하나를 놓쳤고, 신고를 눌렀을 뿐인데 화면이 설명 없이 바뀌었다. 본문 자리를 `EmptyState`로 바꾸고(문구·아이콘은 패턴 문서 예시 그대로) 이동은 `커뮤니티로 돌아가기` 버튼으로 받는다.

토스트 문구 3종은 DOW-1136과 글자까지 동일하게 유지했다(패턴 문서 4절). 새 라이브러리는 들이지 않았다 — `EmptyState`·`lucide-react` 모두 이미 있다.

회귀 테스트의 판정 기준을 "자동 이동했는가"에서 "결과 화면이 뜨고 본문이 치워졌으며 이동은 버튼을 눌렀을 때만 일어나는가"로 바꿨다. 대조군(자동 이동 복원)에서 실패 확인. 전체 `65 files / 650 tests` 통과.
