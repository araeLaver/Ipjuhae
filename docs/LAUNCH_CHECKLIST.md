...
> **선행 조건 — 아래 항목을 돌리기 전에 반드시 확인합니다.** 알림을 켜고 권한을 허용한 직후
> `SELECT count(*) FROM push_tokens` 가 **1 이상**이어야 합니다. 0이면 이 기기에서는 토큰이
> 발급되지 않은 것이고, 그 상태로 아래를 돌리면 `push_tokens`를 보는 항목이 **"행이 없다"로
> 거짓 통과**합니다(정상 동작이라서가 아니라 토큰이 생길 수 없어서). Android는
> `mobile/google-services.json`이 빌드에 포함돼야 토큰이 나옵니다 — 설정 절차와 현재 상태는
> [docs/mobile-fcm-setup.md](./mobile-fcm-setup.md). 근거: DOW-1222.
