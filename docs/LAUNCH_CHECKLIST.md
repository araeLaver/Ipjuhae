...
> **선행 조건 — 토큰 등록 여부를 해당 기기·계정 기준으로 확인합니다.** 알림을 켜고 권한을
> 허용한 직후, 해당 기기의 토큰과 현재 QA 계정에 일치하는 `push_tokens` 행이 있어야 합니다.
> 테이블 전체 `count(*) >= 1`만으로는 다른 기기의 행을 잘못 인정할 수 있습니다.
> 토큰·계정 값은 비공개 로컬 경로에서만 조회하고 보고에는 일치 여부만 남깁니다.
> 등록 행이 없으면 4번(비행기 모드 오류 억제)과 5·6·7번(DB 검증)은 **실행 못함**으로 기록합니다.
