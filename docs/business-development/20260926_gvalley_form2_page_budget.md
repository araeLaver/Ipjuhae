...
## 6. 증빙 판정 수령 후 재산정 (2026-09-26 갱신)

[기술·운영 증빙 검증 결과](/DOW/issues/DOW-1235#document-evidence-verification)를 수령해 3절 원천 B가 해소됐다. 2.2에 이식할 확정 원고는 [서식2 2.2 본문](/DOW/issues/DOW-1234#document-section22-evidence)에 있다.

### 6-1. 재산정 결과 — 최소 5쪽 요건은 충족 경로 확보

| 가정 | 기존 원고 | 원천 A (4절) | 2.2 신규 | **합계** | 최소 5쪽 |
|---|---|---|---|---|---|
| 1,100자/쪽 (보수) | 2.72쪽 | 1.98쪽 | 1.52쪽 | **6.22쪽** | 충족 |
| 1,300자/쪽 (중립) | 2.42쪽 | 1.80쪽 | 1.33쪽 | **5.55쪽** | 충족 |
| 1,500자/쪽 (낙관) | 2.21쪽 | 1.68쪽 | 1.20쪽 | **5.09쪽** | 충족(여유 0.09쪽) |

**단, 이는 원천 A 4건과 2.2 원고가 모두 통합 원고에 이식된 경우의 값이다.** 현재 이 원고들은 작성만 완료됐고 상위 통합 원고에는 반영되지 않았다. 이식되지 않으면 분량은 2.21~2.72쪽 그대로이며 서류심사 제외 사유가 유지된다. **이식은 사업개발 작업이고 이번 회차의 최우선 항목이다.**

낙관 가정에서 여유가 0.09쪽뿐이라는 점을 그대로 적는다. 표지·유의사항 페이지와 절 제목 주변 여백을 포함하면 실제 출력은 추정보다 늘어나는 방향이지만, 낙관 가정이 실제에 가깝다면 5쪽 충족은 아슬아슬하다. **4장 보드 입력(원천 C)이 도착하기 전에는 요건 충족을 확정으로 판정하지 않는다.**

### 6-2. 3.3은 여전히 채울 수 없다

증빙 판정 10번이 "증빙 없음(미측정)"으로 확정됐다. 이용자·검증·전환 실적은 프로덕션 집계 자료가 없어 어떤 수치도 쓸 수 없다. 3.3은 검증 계획 서술을 유지하고, 성장전략 30점 구간의 분량은 3.1·3.2·3.4에서 확보한다. 보드가 프로덕션 집계를 제공하면 그때 재작성한다.

### 6-3. 남은 분량 위험 — 10쪽 권장 대비

| 구간 | 배점 | 현재 | 상태 |
|---|---|---|---|
| 1. 문제인식 | 20 | 원천 A 이식 시 약 2.0쪽 | 확보 |
| 2. 해결방안 | 30 | 2.2 이식 시 약 2.4쪽 | **확보** |
| 3. 성장전략 | 30 | 약 1.5쪽 | 3.3 공란으로 미달 |
| 4. 팀 구성 | 20 | 0.07쪽 | **보드 입력 없이는 해소 불가** |
| 5. 기타 | - | 0.08쪽 | 보드 입력 대기 |

배점 100점 중 **여전히 50점 구간(성장전략 일부 + 팀 구성 전체)이 보드 입력에 걸려 있다.** 10쪽 권장 도달의 병목은 이제 기술 증빙이 아니라 보드 입력이다.
