# G밸리 신청 패키지 — 통합 검토 원고 2026-09-26 (7판)

7판에서 [분량 계량 결함 정정](/DOW/issues/DOW-1234)을 반영해 **분량 수치 2건(머리말·5장)을 정정**하고, 분량 보정 원고를 조건부에서 무조건 이식으로 승격했다. 이전 판이 실었던 5.42~6.68쪽은 과대값이다.
...
2026-09-26 5판에서 [증빙 판정](/DOW/issues/DOW-1235#document-evidence-verification)을 반영한 서식2 2.2 확정 원고와 증량 원고 4건(1.3 확장·도표 2·2.1 확장·3.2 게이트·4장 입력 틀)을 이식했다. 이식 전 추정 2.21~2.72쪽으로 **필수 최소 5쪽에 미달**했다.

**2026-09-27 분량 수치 정정.** 5판·6판이 실었던 이식본 실측 5.42~6.68쪽은 **과대 계상값이었다.** 계량 스크립트가 코드펜스 내부의 mermaid 소스를 본문 글자로 세면서, 같은 도표를 고정 분량(3종 x 0.35쪽)으로 한 번 더 더하고 있었다(수정 커밋 `d7d7ab40`). 본문 글자수 5,199자가 아니라 **4,778자**이고, 현재 원고의 정정 실측은 **5.14~6.29쪽**이다. 낙관 가정(1500자/쪽)의 최소 5쪽 여유는 0.42쪽이 아니라 **0.14쪽이었다.**

정정 수치에 따라 **분량 보정 원고(1.3·3.2, 커밋 `878e6523`)를 렌더링 경로와 무관하게 무조건 이식한다.** 조건부 이식 계획을 폐기한 근거는 경로별 실측이다(`scripts/stats/gvalley_fallback_sim.py`, 사업개발 재실행으로 동일 결과 확인).

| 경로 | 본문 글자 | 1100자/쪽 | 1300자/쪽 | 1500자/쪽 |
|---|---|---|---|---|
| 현재 원고 — 도표 이미지, 보정 없음 | 4,778 | 6.29 | 5.63 | 5.14 |
| 도표를 표로 대체, 보정 없음 | 4,778 | 5.84 | 5.18 | **4.69 미달** |
| 도표를 표로 대체 + 보정 원고 | 5,916 | 6.88 | 6.05 | 5.44 |
| 도표 이미지 + 보정 원고 | 5,916 | 7.33 | 6.50 | 5.89 |

보정 원고를 넣으면 어느 경로로 가도 최소 5쪽을 충족하므로, **최소 5쪽 요건은 편집 환경 확보([DOW-1243](/DOW/issues/DOW-1243)·[DOW-1242](/DOW/issues/DOW-1242))에서 분리된다.** 환경 미확보는 이제 서류심사 제외 사유가 아니라 도표 품질·레이아웃·출력 작업량 문제다. 다만 글자/쪽 가정 자체가 추정이고 보정 후에도 낙관 가정 여유는 0.44쪽이므로, **4장 보드 입력이 도착하기 전에는 분량 요건 충족을 확정으로 판정하지 않는다.** 이식 전 2.21~2.72쪽은 도표가 없던 원고의 추정치여서 이번 결함의 영향 범위 밖이지만 재실측하지는 않았다. 산정 근거는 [분량 예산 문서](/DOW/issues/DOW-1234#document-page-budget) 6절이다.
...
- 공식 서식에 본문과 도표를 반영하고 인쇄 가능한 출력물을 만들어야 한다. Markdown 분량으로 공식 서식의 페이지 수를 확정하지 않는다. 정정 실측(2026-09-27)은 현재 원고 5.14~6.29쪽, **분량 보정 원고 이식 후 도표 이미지 경로 5.89~7.33쪽 · 표 대체 경로 5.44~6.88쪽**이며 실제 페이지 수는 출력 후 확정한다. 이전 판이 실었던 5.42~6.68쪽은 계량 결함에 따른 과대값이다(머리말 정정 참조).
- **분량 보정 원고(1.3·3.2)를 이식하지 않은 채 도표를 표로 대체하면 낙관 가정에서 4.69쪽으로 최소 5쪽에 미달한다.** 보정 원고 이식은 렌더링 경로와 무관하게 무조건 수행한다. 편집 단계에서 이 두 절이 빠지지 않았는지 검수 항목으로 확인한다.
...
현재 통합 원고 내용 수정은 완료했으며 공식 서식 편집은 환경 지원 대기다. [편집 환경 지원](/DOW/issues/DOW-1242)은 CEO 담당으로 진행 중이다. **분량 수치 정정(2026-09-27) 이후 최소 5쪽 요건은 이 환경 대기와 분리됐다** — 보정 원고를 무조건 이식하므로 환경이 끝까지 풀리지 않아도 분량 미달로 서류심사에서 제외되지는 않는다. 환경 티켓의 성격은 탈락 위험이 아니라 도표 품질·레이아웃·출력 작업량이다. 신청서 원본은 `docs/business-development/gvalley-2026/official-application.hwp`이며 함께 확보한 HWPX는 공고문이므로 신청서 대체 편집본으로 사용하지 않는다. CMO의 환경 검사는 [검수 보고서](/DOW/issues/DOW-1234#document-form-readiness)를 따른다. CTO 증빙 판정과 보드 필수 입력도 대기 중이어서 통합 제출본 작업은 blocked로 관리한다. 환경 연결 또는 증빙 도착 시 해당 작업을 재개한다. 최종 제출본 완료 판정은 보류한다.
