...
커밋 도중 이 작업의 staged 파일 3건이 같은 워크트리에서 동시에 돌던 다른 에이전트의
전체 스테이징 커밋(`공개자료 용어 guard`)에 한 번 휩쓸려 들어갔다. 해당 에이전트가
곧바로 amend로 자기 파일만 남기면서 원상복구됐고, 최종적으로 이 작업 파일은 별도
커밋으로 들어갔다. 동시 실행 워크트리에서는 커밋 직후 `git show --name-only`로
내 파일만 들어갔는지 확인하는 절차가 필요하다.
