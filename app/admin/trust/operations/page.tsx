...
function OverviewTab({ overview, counts, failedEvents }: { overview: Overview; counts: Record<string, string>; failedEvents: number }) {
  const cards = [
    ...
    { label: '보관 조치', value: counts.pending_retention ?? '0', icon: Archive, tone: 'bg-muted text-stone-800' },
  ]
...
function QueueRow({ title, detail, badge, action, onAction }: { title: string; detail: string; badge: string; action?: string; onAction?: () => void }) { return <div className="flex flex-col justify-between gap-4 rounded-2xl border border-gray-200 bg-muted/50/60 p-4 sm:flex-row sm:items-center"><div className="min-w-0"><p className="truncate text-sm font-semibold text-gray-900">{title}</p><p className="mt-1 text-xs text-gray-500">{detail}</p></div><div className="flex items-center gap-2"><span className="rounded-full bg-gray-900 px-2.5 py-1 text-[10px] font-bold uppercase text-white">{badge}</span>{action && onAction ? <button onClick={onAction} className="rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-xs font-semibold text-gray-700 hover:bg-muted">{action}</button> : null}</div></div> }

function Dialog({ title, onClose, children }: { title: string; onClose: () => void; children: ReactNode }) { return <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-950/50 p-4 backdrop-blur-sm"><div role="dialog" aria-modal="true" className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-white shadow-2xl"><div className="sticky top-0 flex items-center justify-between border-b border-gray-100 bg-white/95 px-6 py-5 backdrop-blur"><h2 className="font-serif text-2xl font-black">{title}</h2><button onClick={onClose} aria-label="닫기" className="rounded-full p-2 hover:bg-muted"><X className="h-5 w-5" /></button></div><div className="p-6">{children}</div></div></div> }
