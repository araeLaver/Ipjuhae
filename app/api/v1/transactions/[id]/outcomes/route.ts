...
import { ContractOutcomeCode, recordContractOutcome } from '@/lib/trust-engine'
import { getRequestContext } from '@/lib/request-context'
import { jsonError, jsonSuccess } from '@/lib/api-response'
import { withIdempotency } from '@/lib/idempotency'

const schema = z.object({
  outcome: z.enum(['ContractSigned', 'MoveInConfirmed', 'LeaseEnded', 'DepositReturned', 'completed', 'cancelled', 'defaulted', 'disputed', 'renewed']),
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return withIdempotency({
    ...
    handler: async () => {
      ...
      try {
        const outcome = await recordContractOutcome(id, user.id, parsed.data.outcome as ContractOutcomeCode, parsed.data.terms, parsed.data.evidenceIds, parsed.data.occurredAt, getRequestContext(request))
