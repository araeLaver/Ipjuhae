...
// GET /api/landlord/properties/[id] - ë§¤ë¬¼ ?ì„¸ ì¡°íšŒ
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({ error: 'ë§¤ë¬¼??ë¶ˆëŸ¬?¤ëŠ”???¤íŒ¨?ˆìŠµ?ˆë‹¤' }, { status: 500 })
  }
}

// PUT /api/landlord/properties/[id] - ë§¤ë¬¼ ?˜ì •
export async function PUT(
  ...
  try {
    ...
    if (updates.length === 0) {
      return NextResponse.json({ error: '?˜ì •???´ìš©???†ìŠµ?ˆë‹¤' }, { status: 400 })
    }
    ...
    return NextResponse.json({ error: 'ë§¤ë¬¼ ?˜ì •???¤íŒ¨?ˆìŠµ?ˆë‹¤' }, { status: 500 })
  }
}

// DELETE /api/landlord/properties/[id] - ë§¤ë¬¼ ?? œ
export async function DELETE(
  ...
  try {
    ...
    console.error('ë§¤ë¬¼ ?? œ ?¤ë¥˜:', error)
    return NextResponse.json({ error: 'ë§¤ë¬¼ ?? œ???¤íŒ¨?ˆìŠµ?ˆë‹¤' }, { status: 500 })
  }
}
